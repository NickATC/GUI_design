...
btn2 = ttk.Button(window, text = "Now Button 2", command = button2)
btn2.pack(pady = 10)

# 1 Radiobutton
radio_var1 = tk.StringVar()
radio_opt = ttk.Radiobutton(window, text = "Black?", value = radio_var1, command = radio_black)
radio_opt.pack()

radio_var2 = tk.StringVar()
radio_opt = ttk.Radiobutton(window, text = "Or White", value = radio_var2, command = radio_white)
radio_opt.pack()

# 1 Checkbutton
check_var = tk.IntVar()
check_opt = ttk.Checkbutton(window, text = "Tick me!", variable = check_var, command = checkbutton_ok)
check_opt.pack(pady = 20)

# 1 Spinbox
spinbox1 = ttk.Spinbox(window, width = 15, command = spin_changed)
